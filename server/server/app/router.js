...
module.exports = app => {
  ...
  const jwt = app.middleware.token(app);
  ...
  // 收藏书籍
  router.post('/api/book/collect', jwt, controller.book.collect.index);
  // 判断是否已经收藏
  ...
  // 修改暑假书籍数量
  router.post(
    '/api/book/shelf/amountchange',
    controller.book.shelf.amountChange
  );
  // 获得已选书籍的价格和数量
